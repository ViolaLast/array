...
$('#assignImageBtn').click(function () {
    ...
    if (validateEmail(email)) {
        ...
        fetchAndDisplayImage();  // Fetch and display a new image after assigning
    } else {
...
function addImageToDisplay(selectedEmail) {
    ...
    var currentImageUrl = $('#imageWindow img').attr('src');

    if (currentImageUrl) {
        var $image = $('<img>').attr('src', currentImageUrl).addClass('displayed-image');
        $imgDisplay.html('').append($('<p>').text('Selected Email: ' + selectedEmail), $image);
    } else {
